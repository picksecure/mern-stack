  ...
  useEffect(() => {
    if (!userInfo.isAdmin) {
      ...
      socket.on("no admin", (msg) => {
          setChat((chat) => {
              return [...chat, { admin: "There is currently no one online at the moment. We will get back to you as soon as possible!" }];
          })
      })
        ...
        <div className="chat-form">
          <div className="cht-msg">
            ...
            {chat.map((item, id) => (
              <div key={id}>
                {item.client && (
                        <p className="bg-primary p-3 ms-4 text-light pill">
