...
const getProducts = async (req, res, next) => {
  try {
    ...
    if (req.query.price) {
      ...
      let barcodeQueryCondition = {};
      if (req.query.barcode) {
          queryCondition = true;
          barcodeQueryCondition = { barcode: { $lte: Number(req.query.barcode) } };
      }
...
const getBestsellers = async (req, res, next) => {
  try {
    const products = await Product.aggregate([
        { $sort: { category: 1, featured: -1 } },
      {
          $group: { _id: "$category", doc_with_max_featured: { $first: "$$ROOT" } },
      },
        { $replaceWith: "$doc_with_max_featured" },
        { $match: { featured: { $gt: 0 } } },
...
const CartGetProducts = async (req, res, next) => {
    try {
        const products = await Product.find({})
            .sort({ category: 1 })
            .select(" count");
...
const adminDeleteProductImage = async (req, res, next) => {
  ...
  try {
    const path = require("path");
    const finalPath = path.resolve("../frontend/public/images") + imagePath;
